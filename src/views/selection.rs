...
pub fn select_partiture_view(settings: &CustomSettings) -> Element<AppMessage> {
    ...
    let mut partiture_column: Column<AppMessage> = column![];
    ...
    for partiture in &partitures {
        if let Some((name, _data)) = partiture.as_object().and_then(|obj| obj.iter().next()) {
            ...
            let partiture_button: Button<AppMessage> = Button::new(Text::new(info))
    ...
    let back_button: Button<AppMessage> = Button::new(Text::new("Volver al menú"))
